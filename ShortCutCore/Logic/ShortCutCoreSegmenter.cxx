...
void ShortCutCoreSegmenter::DoSegmentation() {
    ...
    cv::resize(m_imSrc, m_imSrcSample, cv::Size(m_imSrc.cols * m_SAMPLERATE, m_imSrc.rows * m_SAMPLERATE),
               cv::INTER_LINEAR);
    cv::resize(m_imLab, m_imLabSample, cv::Size(m_imLab.cols * m_SAMPLERATE, m_imLab.rows * m_SAMPLERATE),
               cv::INTER_NEAREST);
    ...
    cv::createTrackbar("Feature Strength", m_WindowNameFull, &indColorTh, m_COLOR_SLIDER_MAX, On_TrackColorThreshold,
                       this);
    cv::waitKey();
