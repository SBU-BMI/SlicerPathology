...
class ShortCutOptions(EditorLib.LabelEffectOptions):
    ...
    def parameter_changed(self, name, value):
        ...
        """
        # Placeholder.
        # If we ever check for parameter value (defs are currently commented out),
        # we should have one function def that accepts a parameter name and value,
    ...
    def onStartBot(self):
        ...
        if hasattr(slicer.modules, 'TCGAEditorBot'):
            ...
            TCGASegBot(self)
            # save the function that stops bot, destroys FastGrowCut segmenter, if things go wrong
            slicer.modules.TCGAEditorBot.logic.emergencyStopFunc = self.botEstop

            if self.botButton:
                self.botButton.text = "Stop Quick TCGA Segmenter"
                # We're not doing segmentation in this script.
                ...
                self.currentMessage = "Quick TCGA Segmenter started. Go to PaintEffect to edit label image, or press 'S' to start global segmentation process."
                slicer.util.showStatusMessage(self.currentMessage)
...
class TCGASegBot(object):  # stays active even when running the other editor effects
    ...
    def start(self):
        # sliceLogic = self.sliceWidget.sliceLogic()  # not used here.
...
class ShortCutTool(LabelEffect.LabelEffectTool):
    ...
    def process_event_1(self, caller=None, event=None):
        """
        Original recipe.
        Handle events from the render window interactor.
        :param caller: 
...
class ShortCutLogic(LabelEffect.LabelEffectLogic):
    ...
    def init_ShortCut(self):
        ...
        # alter what is background/foreground in editor)
        # Confused about how info propagates UI array to UIVol, not the other way, NEEDS AUTO TESTS
        ...
        # if red slice doesn't have a label or image, go no further
        ...
        if not self.sliceViewMatchEditor(
                self.sliceLogic):  # do nothing, exit function if user has played with images
            if self.emergencyStopFunc:
                self.emergencyStopFunc()
            return
    ...
    def runQTCGA_Segmentation(self):
        ...
        if self.bEditTCGA:
            ...
            self.qTCGAMod.Run_QTCGA_Segmentation()  # vtk code
    ...
    def resetQTCGAFlag(self):
        ...
        self.currentMessage = "Quick TCGA: parameters have been reset. Go to PaintEffect to initialize labels and press 'S' to start global segmentation"
    ...
    def destroy(self):
        ...
        # print "self.qtkeydefsQTCGA", self.qtkeydefsQTCGA
        for i in range(len(
    ...
    def sliceViewMatchEditor(self, sliceLogic):
        # if self.dialogBox==type(None):  # something deleted the dialogBox, this function then breaks, bail
        #     if self.emergencyStopFunc:
        #         self.emergencyStopFunc()
        #     return False
        ...
        # If sizes don't match up(doing this b/c can't reach HelperBox parameters
        if not (dimImg[0] == dimLab[0] and dimImg[1] == dimLab[1] and dimImg[2] == dimLab[2]):
    ...
    def QTCGAChangeLabelInput(self, caller, event):
        ...
        if not self.sliceViewMatchEditor(self.sliceLogic):
            return  # do nothing, exit function

    def entranceCursorDetect(self, caller, event):
        # interactor = caller  # Should be called by the slicer interactor...
        # self.interactor = interactor
        # self.sw = self.swLUT_growcut[interactor]
        # This is a hack, look at init function, self.sliceLogic already defined as just "Red" slice:
        # self.sliceLogic = self.sw.sliceLogic()
    ...
    def updateShortCutROI(self, caller, event):
        ...
        if event == "LeftButtonPressEvent":
            if self.bEditShortCut:
                ...
                if ijk[0] > self.SCutROIRad and ijk[0] < self.volSize[0] - self.SCutROIRad and ijk[
                    1] > self.SCutROIRad and ijk[1] < self.volSize[1] - self.SCutROIRad:
                    self.qSCutROIArray[:] = 0
                    # self.qSCutROIArray[np.ix_([ijk[0] - self.SCutROIRad, ijk[0] + self.SCutROIRad], [ijk[1] - self.SCutROIRad, ijk[1] + self.SCutROIRad], [0, 1])] = 2
                    self.qSCutROIArray[0, ijk[1] - self.SCutROIRad:ijk[1] + self.SCutROIRad,
                    ijk[0] - self.SCutROIRad:ijk[0] + self.SCutROIRad] = 2
                    # print ("EditROI")
...
class ShortCutEffect:
    ...
    def __init__(self, parent):
        ...
        parent.contributors = [
            "Liangjia Zhu, Erich Bremer, Joel Saltz, Allen Tannenbaum (Stony Brook University)"]  # insert your name in the list
...
def bind_view_observers(handlerFunc):
    ...
    for nodeIndex in xrange(sliceNodeCount):
        ...
        if sliceWidget:  # add observers and keep track of tags
